...
dynamo.setupLogger = function(logger) {
    dynamo.log = logger;
}
...
internals.compileModel = function (name, schema) {
  ...
  var tableName = name; //name.toLowerCase() + 's';

  var table = new Table(tableName, schema, serializer, internals.loadDocClient(), dynamo.log);
