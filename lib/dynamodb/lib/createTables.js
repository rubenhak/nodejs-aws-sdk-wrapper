  ...
  model.describeTable(function (err, data) {
    if(_.isNull(data) || _.isUndefined(data)) {
      ...
      return model.createTable(options, function (error) {
        ...
        if(error) {
          model.log.warn('failed to create table %s: %s', tableName, error, {err : error});
      ...
      model.updateTable(function (err) {
        if(err) {
          model.log.warn('failed to update table %s: %s', tableName, err, {err : err});
          return callback(err);
        }
