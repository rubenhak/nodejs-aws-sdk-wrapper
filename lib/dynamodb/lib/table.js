...
Table.prototype.sendRequest = function (method, params, callback) {
  ...
  self.log.info('dynamo %s request', method.toUpperCase(), {params : params});
  driver[method].call(driver, params, function (err, data) {
    var elapsed = Date.now() - startTime;

    if (err) {
      self.log.warn('dynamo %s error', method.toUpperCase(), {err : err});
      return callback(err);
    } else {
      self.log.info('dynamo %s response - %sms', method.toUpperCase(), elapsed, {data : data});
