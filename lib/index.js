...
class AWSClient
{
    ...
    get DynamoDB() {
        var dynamoDb = this.getAwsService('dynamodb', true);
    ...
    getAwsService(name, skipUnpromise)
    {
        if (!(name in this._awsServices)) {
            ...
            this._constructWrappedService(awsServiceInfo, skipUnpromise);
    ...
    _constructWrappedService(awsServiceInfo, skipUnpromise)
    {
        var throttlerConfig = this._throttlerConfig.default;
        if (awsServiceInfo.name in this._throttlerConfig.byService)
        {
            throttlerConfig = this._throttlerConfig.byService[awsServiceInfo.name];
        }

        awsServiceInfo.throttler = new Throttler(this._logger.sublogger('Throttler' + awsServiceInfo.name),
                                                 throttlerConfig.interval,
                                                 throttlerConfig.number);
