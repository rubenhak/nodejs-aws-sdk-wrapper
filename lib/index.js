...
class AWSClient
{
    ...
    get EcsService() {
        return this._getMyService('./services/ecs-service');
    }
    ...
    getRawAwsService(name)
    {
        ...
        this._logger.info('Constructing RAW AWS service: %s...', name, this._serviceConfig);
    ...
    {
        this._logger.info('[_constructWrappedService] %s begin...', awsServiceInfo.name);
        ...
        specificService = _.cloneDeep(specificService);
        ...
        {
            this._logger.info('[_constructWrappedService] %s is not wrapped', awsServiceInfo.name);
        ...
        this._logger.info('[_constructWrappedService] %s config is: ', awsServiceInfo.name, specificService);

        var handler = {
            get: (target, propKey, receiver) => {
                const origMethod = target[propKey];
                return (...args) => {
                    this._onAwsAction(awsServiceInfo.name, propKey, args);
    ...
    _onAwsAction(serviceName, actionName, actionArgs)
    {
        for(var x of this._actionListeners)
        {
            x(serviceName, actionName, actionArgs);
        }
    }

    monitorActions(cb)
    {
        this._actionListeners.push(cb);
    }
