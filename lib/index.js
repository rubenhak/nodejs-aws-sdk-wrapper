...
class AWSClient
{
    constructor(logger, credentialProfile, region, credentialsPath)
    {
        this._logger = logger;
        this._credentialProfile = credentialProfile;
        this._region = region;
        this._credentialsPath = credentialsPath;

        const AWS = require('aws-sdk');
        AWS.config.update({
            credentials: new AWS.SharedIniFileCredentials({profile: this._credentialProfile}),
            region: this._region
        });
    ...
    getSshPemFilePath(cluster)
    {
        return path.join(this._credentialsPath, cluster, 'aws', this.region, cluster + '.pem');
    }
