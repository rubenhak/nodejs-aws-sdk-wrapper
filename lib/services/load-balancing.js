...
class AWSLoadBalancingClient
{
    ...
    createTargetGroup(config, vpcId, cluster, service, name)
    {
        ...
        params.VpcId = vpcId;
        this._logger.info('Creating TargetGroup %s::%s...', cluster, service);
        this._logger.verbose('Creating TargetGroup... %s', '', params);
        var arn = null;
        return this._elb.createTargetGroup(params).promise()
            .then(result => {
                var tg = result.TargetGroups[0];
                arn = tg.TargetGroupArn;
                this._logger.verbose('TargetGroup created %s', '', tg);
    ...
    queryTargetGroup(arn)
    {
        ...
        this._logger.verbose('Querying TargetGroup %s...', arn);
    ...
    deleteTargetGroup(obj)
    {
        ...
        this._logger.info('Deleting TargetGroup %s...', obj.TargetGroupArn);
        this._logger.verbose('Deleting TargetGroup... %s', '', params);
    ...
    queryAllTargetGroups(cluster, marker, results)
    {
        ...
        this._logger.verbose('Querying TargetGroups for %s...', cluster);
        return this._elb.describeTargetGroups(params).promise()
            .then(result => {
                return Promise.serial(result.TargetGroups, x => this._queryTags(x.TargetGroupArn, x))
                    .then(objects => {
                        for(var x of objects) {
                            if (this._parent.getObjectTag(x, 'berlioz:cluster') == cluster) {
                                results.push(x);
                            }
                        }
    ...
    queryAllLoadBalancers(cluster, marker, results)
    {
        ...
        this._logger.verbose('Querying LoadBalancers for %s...', cluster);
    ...
    queryLoadBalancer(arn)
    {
        ...
        this._logger.verbose('Querying LoadBalancer %s...', arn);
        return this._elb.describeLoadBalancers(params).promise()
            .then(result => {
            ...
            .catch(error => {
                this._logger.warn('There was error querying LoadBalancer: %s. It might have been deleted.', arn, error);
    ...
    createLoadBalancer(config, cluster, service, name, securityGroups, subnets)
    {
        ...
        params.Tags = this._parent.toTagArray({
            'berlioz:cluster': cluster,
            'berlioz:service': service,
            'berlioz:endpoint': name
        });

        this._logger.info('Creating LoadBalancer %s::%s::%s...', cluster, service, name);
        this._logger.verbose('Creating LoadBalancer... %s', '', params);
        return this._elb.createLoadBalancer(params).promise()
            .then(result => {
                var lb = result.LoadBalancers[0];
                this._logger.verbose('LoadBalancer created %s', '', lb);
    ...
    modifySecurityGroups(arn, securityGroups)
    {
        ...
        this._logger.info('Setting LoadBalancer %s security groups...', arn);
        this._logger.verbose('Setting LoadBalancer %s security groups...', arn, params);
    ...
    deleteLoadBalancer(arn)
    {
        ...
        this._logger.info('Deleting LoadBalancer %s...', arn);
        this._logger.verbose('Deleting LoadBalancer .. %s', '', params);
        return this._elb.deleteLoadBalancer(params).promise()
            .then(result => {
                this._logger.verbose('Load Balancer deleted %s', '', result);
                return this._waitLoadBalancerDelete(arn);
            })
            .then(() => {
                this._logger.info('Load Balancer %s is deleted ', arn);
            })
            ;
    }
    ...
    {
        this._logger.verbose('Waiting LoadBalancer delete %s', arn);

        return Promise.timeout(2000)
            .then(() => this.queryLoadBalancer(arn))
            .then(lb => {
                if (lb) {
                    this._logger.verbose('LoadBalancer is still present %s', '', lb);
                    return this._waitLoadBalancerDelete(arn);
                }
            });
    ...
    queryAllListeners(lbArn, marker, results)
    {
        ...
        this._logger.verbose('Querying LoadBalancer Listeners for %s...', lbArn);
    ...
    createListener(loadBalancerArn, targetGroupArn, config)
    {
        ...
        this._logger.info('Creating LoadBalancer Listener %s => %s...', loadBalancerArn, targetGroupArn);
        this._logger.verbose('Creating LoadBalancer Listener... %s', '', params);
        return this._elb.createListener(params).promise()
            .then(result => {
                var listener = result.Listeners[0];
                this._logger.verbose('Listener created %s', '', listener);
                return listener;
            });
    }
    ...
    deleteListener(arn)
    {
        ...
        this._logger.info('Deleting LoadBalancer Listener %s...', arn);
        this._logger.verbose('Deleting LoadBalancer Listener... %s', '', params);
        return this._elb.deleteListener(params).promise()
            .then(result => {
                this._logger.verbose('Listener deleted %s', '', result);
            });
    }
    ...
    queryTargetHealth(targetGroupArn)
    {
        ...
        this._logger.verbose('Querying Target Health for %s...', targetGroupArn);
        return this._elb.describeTargetHealth(params).promise()
            .then(result => {
                ...
                this._logger.verbose('Targets for %s', targetGroupArn, targets);
                return targets;
            });
    }
    ...
    registerTargetGroupMember(targetGroupArn, instanceId, port)
    {
        ...
        this._logger.verbose('Registering Target Group Member for %s...', targetGroupArn);
        this._logger.verbose('Registering Target Group Member for %s...', targetGroupArn, params);
        return this._elb.registerTargets(params).promise()
            .then(result => {
                this._logger.verbose('Target Group Member registered %s', '', result);
            });
    }
    ...
    deregisterTargetGroupMember(targetGroupArn, instanceId, port)
    {
        ...
        this._logger.verbose('Deregistering Target Group Member for %s...', targetGroupArn);
        this._logger.verbose('Deregistering Target Group Member for %s...', targetGroupArn, params);
        return this._elb.deregisterTargets(params).promise()
            .then(result => {
                this._logger.verbose('Target Group Member deregistered %s', '', result);
            });
    }
    ...
    _queryTags(arn, obj)
    {
        ...
        this._logger.silly('Querying obj tags %s...', arn);
    ...
    _setupTags(arn, obj, newTags) {
        ...
        for(var tagName of _.keys(newTags)) {
            ...
            if (this._parent.getObjectTag(obj, tagName) != tagValue) {
                tags.push({
                    Key: tagName,
                    Value: tagValue
                });
