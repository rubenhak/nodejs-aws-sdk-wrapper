...
class AWSSubnetClient
{
    ...
    {
        this.logger.verbose('Fetching Subnet  %s :: %s', vpc, cluster);
        ...
        return this._ec2.describeSubnets(params).promise()
            ...
            .then(result => {
                this.logger.debug('Fetched Subnet. %s', '', result);
                return result;
            });
    }
    ...
    queryAll(cluster)
    {
        this.logger.verbose('Fetching Subnets %s', cluster);
        var params = {
            Filters: [
                {
                    Name: 'tag:berlioz:cluster',
    ...
    {
        return this.parent.Ec2utils.setTags(subnet.SubnetId, subnet.Tags, { Name : cluster, 'berlioz:cluster': cluster })
            .then(() => {
                var params = {
                    SubnetId: subnet.SubnetId,
                    MapPublicIpOnLaunch: {
