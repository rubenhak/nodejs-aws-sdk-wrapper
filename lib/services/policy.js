...
class AWSPolicyClient
{
    ...
    remove(policy)
    {
        ...
        this.logger.info('Deleting Policy %s...',  policy.PolicyName);
        this.logger.verbose('Deleting Policy %s...', '', params);
        return this._iam.deletePolicy(params).promise()
            .then(data => {
                this.logger.verbose('Policy version %s deleted.',  policy.PolicyName);
            });
    }
