...
class AWSHostedZoneClient
{
    ...
    {
        this._logger.verbose('Fetching LogGroup %s...', cluster);
        ...
        return this._query(cluster)
            ...
            .then(zone => {
                this._logger.debug('Fetched Hosted Zone %s', '', zone);
                return zone;
            });
    }
    ...
    queryById(id) {
        ...
        this._logger.verbose('Querying HostedZone %s...', id);
            ...
            .then(zone => {
                return this.listRecordSets(zone.Id)
                    .then(records => {
                        ...
                        this._logger.verbose('HostedZone: %s', '', zone);
                        return zone;
                    });
            });
    }
    ...
    create(vpcId, name, tags) {
        ...
        this._logger.info('Creating hosted zone for %s...', params.Name);
        this._logger.verbose('Creating hosted zone ...%s', '', params);
        var zoneId = null;
        return this._route53.createHostedZone(params).promise()
            .then(result => {
                this._logger.verbose('Created hosted zone %s', '', result);
    ...
    registerRecord(zone, domain, address) {
        ...
        this._logger.info('Registering hosted zone record %s -> %s...', dnsName, address);
        this._logger.debug('Registering hosted zone record...%s', '', params);
        return this._route53.changeResourceRecordSets(params).promise()
            .then(data => {
                this._logger.debug('Created hosted zone record %s', '', data);
    ...
    listRecordSets(hostedZoneId)
    {
        ...
        this._logger.silly('Querying RecordSets from %s...', hostedZoneId);
        return this._route53.listResourceRecordSets(params).promise()
            .then(data => {
                this._logger.silly('RecordSets: %s', '', data);
    ...
    deleteRecordSet(hostedZoneId, recordSet)
    {
        ...
        this._logger.info('Deleting RecordSet %s from %s...', hostedZoneId, recordSet.Name);
        return this._route53.changeResourceRecordSets(params).promise()
            .then(data => {
                this._logger.verbose('RecordSet deleted %s', '', data);
            });
    }
    ...
    remove(hostedZoneId) {
        ...
        this._logger.info('Deleting HostedZone %s...', hostedZoneId);
        return this.listRecordSets(hostedZoneId)
            .then(recordSets => {
                this._logger.debug('RecordSets %s', '', recordSets);
    ...
    delete(hostedZoneId) {
        ...
        return this._route53.deleteHostedZone(params).promise()
            .then(result => {
                this._logger.info('HostedZone %s deleted', hostedZoneId);
            });
    }
    ...
    queryAll(result, marker)
    {
        ...
        return this._route53.listHostedZones(params).promise()
            .then(data => {
                // this._logger.info('HostedZoneQueryResult%s', '', data);
    ...
    _setTags(hostedZoneId, tags)
    {
        ...
        return this._route53.changeTagsForResource(params).promise()
            .then(result => {
                this._logger.info('HostedZone tag change result%s', '', result);
                return null;
            });
    }
    ...
    getTags(hostedZoneId) {
        ...
        return this._route53.listTagsForResource(params).promise()
            .then(data => {
                this._logger.silly('getTags result%s', '', data);
                var tags = {};
