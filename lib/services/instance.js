...
class AWSInstanceClient
{
    ...
    run(config, waitStabilize)
    {
        ...
        this._logger.info('Creating Instance...%s', '', params);
        return this._ec2.runInstances(params).promise()
            .then(data => {
                this._logger.info('Run Instance Result%s', '', data);
    ...
    terminate(instanceId)
    {
        ...
        this._logger.info('Terminating Instance %s...', instanceId);
        return this._ec2.terminateInstances(params).promise()
            .then(data => {
                this._logger.info('Terminating Instance Result%s', '', data);
                // var instance = data.Instances[0];
                // return instance;
            })
    ...
    waitInstanceStable(instanceId, instance)
    {
        if (this._isInstanceStable(instance)) {
            this._logger.info('Instance %s is stable', instanceId);
            return instance;
        }

        this._logger.info('Waiting Instance %s to be stable...', instanceId);
