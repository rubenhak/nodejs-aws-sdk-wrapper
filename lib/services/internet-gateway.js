...
class AWSInternetGatewayClient
{
    ...
    // {
    //     this._logger.verbose('Fetching InternetGateway %s :: %s...', vpc.VpcId, cluster);
    //
    ...
    //         })
    //         .then(result => {
    //             this._logger.debug('Fetched InternetGateway. %s', '', result);
    //             return result;
    //         });
    // }
    ...
    {
        var igId = null;
        this._logger.info('Creating InternetGateway...');
        return this._ec2.createInternetGateway({}).promise()
            ...
            .then(ig => {
                return this._parent.Ec2utils.setTags(igId, ig.Tags, { Name : cluster, 'berlioz:cluster': cluster });
            })
            .then(() => {
    ...
    delete(igId) {
        ...
        this._logger.info('Deleting InternetGateway %s...', igId);
    ...
    attach(igId, vpcId)
    {
        ...
        this._logger.info('Attaching InternetGateway %s to %s...', igId, vpcId);
    ...
    detach(igId, vpcId)
    {
        ...
        this._logger.info('Detaching InternetGateway %s from %s...', igId, vpcId);
