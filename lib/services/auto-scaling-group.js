...
class AWSAutoScalingGroupClient
{
    ...
    // {
    //     this.logger.verbose('Fetching AutoScalingGroup %s...', cluster);
    //
    ...
    //         })
    //         .then(result => {
    //             this.logger.debug('Fetched AutoScalingGroup. %s', '', result);
    //             return result;
    //         });
    // }
    ...
    createForCluster(lc, subnetIds, zones, cluster, zone)
    {
        ...
        this.logger.info('Creating AutoScalingGroup %s...', cluster);
        this.logger.verbose('Creating AutoScalingGroup... %s', '', params);
        return this._autoscaling.createAutoScalingGroup(params).promise()
            .then(result => {
                this.logger.verbose('AutoScalingGroup created %s', '', result);
        ...
        return this._autoscaling.describeAutoScalingGroups(params).promise()
            .then(data => {
                for(var asg of data.AutoScalingGroups) {
                    if (this.parent.getObjectTag(asg, 'berlioz:cluster') == cluster) {
                        results.push(asg);
                    }
                }
                if (data.NextToken) {
    ...
    _waitAutoScalingGroupStabilize(asgName, capacity) {
        this.logger.info('AutoScalingGroup %s stabilizing...', asgName);
    ...
    _waitAutoScalingGroupStabilizeX(resolve, reject, asgName, capacity) {
        try {
            this.query(asgName)
            .then(data => {
                this.logger.info('AutoScalingGroup %s stabilizing...', asgName, data);
                if (!data)
                {
                ...
                var clusterName = this.parent.getObjectTag(data, 'berlioz:cluster');
                this.logger.info('AutoScalingGroup %s cluster=%s', asgName, clusterName);

                this.logger.info(`AutoScalingGroup %s stabilizing. Desired=%s. Actual=%s.`, asgName, data.DesiredCapacity, data.Instances.length);
                        ...
                        }))
                    {
                        this._ecs.listContainerInstances({ cluster: clusterName }).promise()
                            .then(ciData => {
                                ...
                                {
                                    this.logger.info(`AutoScalingGroup. Waiting for container instances...`);
                                    setTimeout( () => {
                                        this._waitAutoScalingGroupStabilizeX(resolve, reject, asgName, capacity);
                                    }, 5000);
                                }
                                ...
                                {
                                    this.logger.info(`AutoScalingGroup. Stabilized.`);
                        ...
                        this.logger.info(`AutoScalingGroup. Waiting for instances...`);
                    }
                } else {
                    this.logger.info(`AutoScalingGroup. Waiting for cluster...`);
                }
    ...
    _waitContainerInstancesStabilize(asgName) {
        this.logger.info(`AutoScalingGroup. Stabilizing Container Instances...`);
        return new Promise((resolve, reject) => {
    ...
    _waitContainerInstancesStabilizeX(resolve, reject, asgName) {
        try {
            this.logger.info(`WaitContainerInstances ...`);
