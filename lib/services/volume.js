...
class AWSVolumeClient
{
    ...
    query(id) {
        this._logger.silly('Volume query %s...', id);
        return this._ec2.describeVolumes({ VolumeIds: [ id ] }).promise()
            .then(data => {
                var volume = data.Volumes[0];
                this._logger.silly('Volume query result: %s', '', volume);
                return volume;
            });
    }
    ...
    create(params) {
        this._logger.info('Creating Volume...');
        this._logger.verbose('Creating Volume... %s', '', params);
        return this._ec2.createVolume(params).promise()
            .then(volume => {
                this._logger.verbose('Volume created %s', '', volume);
    ...
    moveToZone(volume, zone) {
        ...
        return this.createSnapshot(volume.VolumeId)
            .then(snapshot => {
                ...
                this._parent.setTagArrayValue(tags, 'berlioz:sourceVolume', volume.VolumeId);
                this._parent.setTagArrayValue(tags, 'berlioz:sourceSnapshot', snapshot.SnapshotId);
                var params = {
    ...
    _waitVolumeState(volume, states) {
        this._logger.verbose('Volume %s is %s', volume.VolumeId, volume.State);
        if (states.includes(volume.State)) {
            return volume;
        }

        this._logger.verbose('Waiting volume %s state %s...', volume.VolumeId, states);
    ...
    _waitVolumeAttachStabilize(volume) {
        for(var attachment of volume.Attachments) {
            this._logger.verbose('Volume %s :: %s is %s', volume.VolumeId, attachment.InstanceId, attachment.State);
        }
        ...
        this._logger.verbose('Waiting volume %s attachment stabilize...', volume.VolumeId);
        return Promise.timeout(2000)
            .then(() => {
                return this.query(volume.VolumeId);
            })
    ...
    createSnapshot(volumeId) {
        ...
        this._logger.info('Creating snapshot for %s...', volumeId);
        this._logger.verbose('Creating snapshot... %s', '', params);
        return this._ec2.createSnapshot(params).promise()
            .then(snapshot => {
                this._logger.verbose('Snapshot created %s', '', snapshot);
                return this._waitSnapshotReady(snapshot);
            });
    }

    _waitSnapshotReady(snapshot) {
        this._logger.verbose('Snapshot %s is %s', snapshot.SnapshotId, snapshot.State);
        if (snapshot.State === 'completed') {
            return snapshot;
        }

        this._logger.verbose('Waiting snapshot %s ready...', snapshot.SnapshotId);
    ...
    deleteSnapshot(snapshotId)
    {
        ...
        this._logger.info('Deleting snapshot %s...', snapshotId);
        return this._ec2.deleteSnapshot(params).promise()
            .catch(reason => {
                this._logger.error('Could not delete shapshot %s', snapshotId, reason);
            });
    }
    ...
    delete(volumeId)
    {
        ...
        this._logger.info('Deleting volume %s...', volumeId);
        return this._ec2.deleteVolume(params).promise()
            .catch(reason => {
                this._logger.error('Could not delete volume %s', volumeId, reason);
            });
    }
    ...
    performVolumeStuffCleanup(volume) {
        return Promise.resolve()
            .then(() => {
                var snapshotId = this._parent.getObjectTag(volume, 'berlioz:sourceSnapshot');
                if (snapshotId) {
                    return this.deleteSnapshot(snapshotId);
                }
            })
            .then(() => {
                var volumeId = this._parent.getObjectTag(volume, 'berlioz:sourceVolume');
                if (volumeId) {
                    return this.delete(volumeId);
                }
            })
            .then(() => {
                return this._parent.Ec2utils.deleteTags(volume.VolumeId,
                                                        volume.Tags,
    ...
    attach(volumeId, instanceId, deviceName)
    {
        ...
        this._logger.info('Attaching volume %s to %s at %s...', volumeId, instanceId, deviceName);
        return this._ec2.attachVolume(params).promise()
            .then(result => {
                this._logger.info('Volume attach result %s', '', result);
                return;
            })
            .then(() => {
    ...
    detach(volume, instanceId)
    {
        ...
        this._logger.info('Detaching volume %s...', volume.VolumeId);
        return this._ec2.detachVolume(params).promise()
            .then(result => {
                this._logger.info('Volume detach result %s', '', result);
                return result;
            })
            .then(() => {
                return this.query(volume.VolumeId);
    ...
    mountInstanceDataVolume(instance, drive, targetPath, sshPemFilePath) {
        this._logger.info('Mounting %s to %s on %s...', drive, targetPath, instance.InstanceId)
        ...
        script.appendLine();
        // process.exit(111);
        return this._executeInstanceScript(instance, script, sshPemFilePath);
    }

    unmountInstanceDataVolume(instance, drive, sshPemFilePath) {
        this._logger.info('Unmounting %s on %s...', drive, instance.InstanceId)
        var vdrive = drive.replace('/dev/sd', '/dev/xvd');
        var script = new StringBuilder();
    ...
    _executeInstanceScript(instance, script, sshPemFilePath) {
        this._logger.info('_executeInstanceScript:');
        this._logger.info(script.toString());

        return new Promise((resolve, reject) => {
            var conn = new SSHClient();
            conn.on('ready', () => {
                this._logger.info('Client :: ready');
                conn.shell((err, stream) => {
                    ...
                    stream.on('close', () => {
                        this._logger.info('Stream :: close');
                        conn.end();
                        resolve();
                    }).on('data', (data) => {
                        this._logger.info('STDOUT: ' + data);
                    }).stderr.on('data', (data) => {
                        this._logger.info('STDERR: ' + data);
                    });
                ...
                port: 22,
                username: 'ec2-user',
                privateKey: require('fs').readFileSync(sshPemFilePath)
            });
